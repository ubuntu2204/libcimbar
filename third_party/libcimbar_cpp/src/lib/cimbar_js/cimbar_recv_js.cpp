...
namespace {
	...
	// Summarize brightness/contrast of a frame, for decode diagnostics.
	// A near-uniform frame (tiny stddev) or a very dark/blown-out one usually
	// means the barcode isn't actually visible / in focus in the capture.
	std::string describe_image(const cv::UMat& img)
	{
		cv::UMat gray;
		if (img.channels() >= 3)
		...
		return fmt::format("bright[min={}, max={}, mean={:.1f}, stddev={:.1f}]",
			(int)mn, (int)mx, mean[0], stddev[0]);
	}
}
...
int cimbard_scan_extract_decode(const uchar* imgdata, unsigned imgw, unsigned imgh, int format, uchar* bufspace, unsigned bufsize)
{
	...
	// Locate the 4 corner anchors and deskew the frame.
	//
	// We first try the fast global (Otsu) threshold. Photos of screens often
	// have a bright background, glare, or letterbox borders that skew a single
	// global threshold so the anchors are lost -> scan() returns < 4 anchors
	// and the whole decode fails with -3. In that case we retry with an
	// adaptive (local mean) threshold, which is far more tolerant of uneven
	// lighting -- the common real-world capture case.
	const cimbar::vec_xy imageSize{cimbar::Config::image_size_x(), cimbar::Config::image_size_y()};
	const unsigned anchorSize = cimbar::Config::anchor_size();

	bool shouldPreprocess = true;
	{
		Timer t(_tScanExtract);

		Scanner scannerFast(img);               // fast=true  -> global Otsu threshold
		std::vector<Anchor> points = scannerFast.scan();
		const size_t fastCount = points.size();
		size_t adaptiveCount = 0;
		bool usedAdaptive = false;

		if (points.size() < 4)
		{
			Scanner scannerAdaptive(img, false);   // fast=false -> adaptive threshold
			std::vector<Anchor> adaptivePoints = scannerAdaptive.scan();
			adaptiveCount = adaptivePoints.size();
			if (adaptivePoints.size() >= 4)
			{
				points.swap(adaptivePoints);
				usedAdaptive = true;
			}
		}
		...
		if (points.size() < 4)
		{
			// Barcode not found. Report *why* so the caller can guide the user
			// (move closer to fill the frame, hold steadier, add light, etc.).
			// Retrieve this via cimbard_get_report().
			_reporting = fmt::format(
				"scan FAIL: found {} anchor(s) (need 4) [fast={}, adaptive={}]; "
				"frame {}x{} fmt={}; {}",
				points.size(), fastCount, adaptiveCount, imgw, imgh, format,
				describe_image(img));
			return -3;
		}

		Corners corners(points);
		Deskewer de(0, imageSize, anchorSize);
		img = de.deskew(img, corners);

		_reporting = fmt::format(
			"scan OK: 4 anchors [fast={}, adaptive={}, usedAdaptive={}, granular={}]",
			fastCount, adaptiveCount, usedAdaptive, corners.is_granular_scale(imageSize));
	}
